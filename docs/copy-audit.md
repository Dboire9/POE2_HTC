...
| 89 | "Tip · buy one already rolled" / "Magic · one prefix, no suffix — worth up to X — more than a plain one" | `StandInTip.tsx` | **OK — from the solve** | `standIns` (tablets.ts): for every Magic (≤1 a side) and Rare (≤2 a side) junk-only state, worth = plain + V(white) − V(state), V read off `markov.routes`; shown when ≥ 90% of the plain price. Search: rarity + `FULL_USES` + `standInFilters` (empty-prefix/suffix pseudo stats; a Rare's are true under a 2- or 3-a-side count, so `≈`) |
