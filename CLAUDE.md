...
- **A regular essence needs a Magic item and turns it Rare; a Perfect essence works on a Rare** (`perfectEssenceProbability` has no rarity gate) — but they grant from **disjoint mod pools**: `source: 'essence'` (317 mods) vs `source: 'perfect_essence'` (363), zero id overlap, both living in `base.pools.essence`. So "one essence-only mod per craft" is a real limit and a Perfect Essence cannot lift it.
