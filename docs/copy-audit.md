...
| 71 | "Super jackpot" / "Jackpot" / "Very good" / "Good" and "~20 div when Dorian checked, 23 Sep" | `TabletResult.tsx` watch list | **OK — a dated reading, and it says whose** | The tiers and prices of `data/tablets/valuable.json`, read by Dorian off the trade site (instant buyout, 10 uses left) on its `pricedOn` date, per tablet. The date is printed because the price ages; the row's own box is for today's price |
