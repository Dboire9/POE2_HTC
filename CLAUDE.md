...
- **Known hole: armour desecration is unplannable in Item mode.** `markovActions.ts` only emits desecrate actions under `if (desecratable && bossTargetable)`, so on armour the MDP has no desecrate action and reports `feasible: false`. The gate is correct (the omen really is weapon/jewellery-only); the fix is adding the *untargeted* draw the game does allow, via `desecrationBossAnySideProbability`.
