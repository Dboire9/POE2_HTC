...
Why the code is the way it is. `CLAUDE.md` states each rule in a line; this file keeps the bug, the
measurement or the decision behind it. **Read the section for an area before changing code there** —
several of these record a design that was built, measured and reverted, and a rebuild would repeat it.

Measurements and divergences are logged in full in `docs/validation.md`; every claim the UI makes, with
what enforces it, in `docs/copy-audit.md`. The entries dated up to 2026-09-23 were moved here from
`CLAUDE.md` on 2026-09-24 and regrouped by area, with their wording kept.

## Contents

- [Architecture and conventions](#architecture-and-conventions)
- [Game mechanics: orbs and omens](#game-mechanics-orbs-and-omens)
- [Game mechanics: desecration and bones](#game-mechanics-desecration-and-bones)
- [Game mechanics: essences](#game-mechanics-essences)
- [Game mechanics: what an item can hold (limits, crafted mods, runes)](#game-mechanics-what-an-item-can-hold-limits-crafted-mods-runes)
- [Data pipeline and what ships](#data-pipeline-and-what-ships)
- [Reading real items: pastes, profiles, streamer gear](#reading-real-items-pastes-profiles-streamer-gear)
- [The MDP solver (true expected cost)](#the-mdp-solver-true-expected-cost)
- [The step planners](#the-step-planners)
- [Prices](#prices)
- [The app](#the-app)
- [Tablets](#tablets)
- [Deploy, monitoring and security](#deploy-monitoring-and-security)
- [Testing and measuring](#testing-and-measuring)

## Architecture and conventions

The long form of CLAUDE.md's layout, commands and core rules.

- `src/` — React 19 + Vite + Tailwind frontend. The only view is the Engine Lab (`src/features/engine/`), driven by the browser facade `src/lib/engine.ts`. Four tabs: **Plan from scratch** (craft from a white base), **I have an item** (craft from an item you already hold), **Tablets** and **Streamer gear**.
...
- **Electron was removed on 2026-08-22**: the app is web-only. The packaged desktop build could not
  coexist with the solver Web Worker (Chromium blocks module Workers from `file://`).
...
- **Data lives in JSON, never in source.** The hardcoded Java/C mod data is the disease being cured. All new code reads `data/patches/<patch>/`. Never "fix" a probability by editing engine logic when the weight data is the problem — change the data and the generator that produces it. (`weights_overrides.json` exists only under `data/patches/0.5/`, is an empty stub, and **nothing reads it** — `loadPatch` opens `mods.json` and `base_items.json` and nothing else. Earlier versions of CLAUDE.md claimed it "always wins at load time"; that mechanism was never built. Don't plan around it.)
...
## Game mechanics: orbs and omens

Enforced in `packages/engine` (mostly `probability.ts`).
...
- **Orb strength floors are per currency** (`CURRENCY_FLOOR`, `packages/engine/src/types.ts`): Regal and
  Exalted 35/50; Transmutation and Augmentation 55/70 (both confirmed 2026-09-10). `REACHABLE_FLOORS` (`markovActions.ts`) derives from it plus the Ancient
...
## Game mechanics: desecration and bones
...
- **Two grades of bone, and a reroll.** An Ancient bone ("Minimum Modifier Level: 40",
  ...
  signature. The omen is spent only when the reroll is TAKEN (confirmed 2026-09-10), so it is not in
  `stepCost`'s up-front price (`ECHOES_OMEN`, cost.ts): `keepWeights` adds its expected spend and rerolls
  only while the offer is worse than the omen plus a fresh three. Since the player may always decline, the
  omened draw dominates the plain one and REPLACES it wherever the omen is to hand; `published` shows it
  as the plain draw wherever the reroll is never taken, and `PolicyNode.actionCost` carries the spend.
...
## Game mechanics: what an item can hold (limits, crafted mods, runes)
...
- **THE SIX "Can roll …" POOL RUNES ADD MODIFIERS, and they live in `base.pools.rune[<runeId>]`**
  ...
  `RUNE_POOL_ASSUMED_WEIGHT` is 1000 (the maintainer's call, 2026-09-15) and the app discloses it.
...
## Data pipeline and what ships

`tools/refresh/`, `data/patches/`, `shipMods.ts`, `vite.config.ts`.
...
- **PRECURSOR TABLETS ARE ITEMS, NOT A SPECIAL CASE** (2026-09-21). Ritual, Overseer and Temple are
  ...
  list is the maintainer's** (`data/tablets/valuable.json`): four tiers of sets priced on the trade site, each
  ...
  `tablets.test.ts` holds each id to its text; match new lines by TEXT, never by id. **A tablet is solved
  FULL** (the maintainer's call, 2026-09-23: always run with four modifiers): `spare` = every slot not picked, plus
...
- **A stat the game files store finer than the game prints is converted at refresh** (2026-09-23,
  `tools/refresh/displayUnits.mjs`): `_per_minute` ÷ 60 to one decimal, `_permyriad` ÷ 100 to two — the
  suffixes the game's own translation files convert. `refresh.mjs`, `apply_tablets.mjs` and
  `apply_runes.mjs` pass every RePoE range through `shownRange`; poe2db's (Essence, Desecrated) are printed
  already. So `mods.json` holds PRINTED units — never divide again downstream. Read raw, a T1 body armour
  showed "1986–2160 Life Regeneration per second" where the game says 33.1–36, and its trade search found
  nothing.

## Reading real items: pastes, profiles, streamer gear
...
- **TWO MODIFIERS CAN PRINT AS ONE LINE, and no reader of the text can undo it.** The game SUMS
  ...
  **The new characters exposed a PIPELINE gap, first misread here as a data gap.** 11 Rares across four
  ...
  bases by metadata path — so it proves nothing. Fixed the same day by reading base-type twins (Data pipeline
  and what ships), and all 11 resolve. The tab still NAMES any item it cannot read: the job persists `skipped` (Rares and Uniques only — socketed runes in `Chakra` slots
...
## The MDP solver (true expected cost)

`packages/optimizer/src/markov*.ts`. The model first, then what makes it fast, then how its answers are read.
...
- **Trade links are links, never requests** (2026-09-23). `tradeUrl` (`src/lib/tradeLink.ts`) builds a
  pathofexile.com/trade2 search the PLAYER opens; nothing in the app fetches the trade site — GGG's terms
  (§7(c)/(f)) forbid scraping it, and its API takes no cross-origin calls anyway. Stat ids come from
  Exiled Exchange 2's MIT `stats.ndjson` at one pinned commit (`EE2_COMMIT` in
  `tools/refresh/gear_trade_stats.mjs` and `tablet_trade_stats.mjs`), mapped per modifier line into
  `data/trade/gear-stats.json` and `data/tablets/trade-stats.json`. The gear map is a lazy import
...
is under Data pipeline and what ships.

- **A tablet is solved exactly, with no Search effort to pick** (2026-09-23). Its lattice is a few hundred
  states, so the tab sends `smallLattice` (solve.ts → `heuristicSeed` + `exactEvaluation`): every policy
  is costed by a dense linear solve of its chain (`denseSolve.ts`, up to `DIRECT_MAX_STATES` = 1,000
  non-terminal states). The closed-form sweeps alternated forever between two values on a tablet's four
  rarest modifiers (598M ↔ 324M ex).
- **Never an Annulment Orb on a tablet** (the maintainer's call, 2026-09-23): the tab adds `annul` to the
  exclusions on every solve. At ~7 Chaos Orbs apiece it pays only on the rarest pairs with dear plain
  tablets — crafts that lose hundreds of div anyway.
...
- **Stand-ins are Magic only: one prefix, or one suffix** (2026-09-23). A Rare with a single modifier is
  too rare to shop for. Each is worth `plain + V(plain) − V(this one)`, read off the same solve
  (`standIns`). On the trade site the filter is one pseudo stat on the OTHER side —
  `pseudo.pseudo_number_of_empty_<other>_mods` at most 1 (`standInFilters`, checked on the site).
- **Every tablet trade link is instant buyout with at least 10 uses left** (`securable` + `FULL_USES`,
  `tradeLink.ts`).
- **The watch list is the curated tiers plus the player's own sets** (`tabletWatch.ts`, per browser, every
  read in try/catch). A curated set can be hidden, never edited. A row shows its tier only — never a
  remembered price, never a person's name.
- **Tablets are priced in chaos by default** (`tabletPrices.ts`) — that is what they trade in. A price
  can be typed in ex, chaos or div, and the shown unit is remembered per browser.
...
- **Work lands on `beta`, production is `main`, and the gap between them is the point** (2026-09-02,
  ...
  **The daily price refresh is a deliberate exception and merges straight to `main`.** It is data, not
  code; it is guarded by the whole test suite plus a market-depth check running in its own job
  before it merges; and routing it through `beta` would mean prices never reach players without a human, which is
