...
public class CraftingStep_Util {
    ...
    ) 
	{
		...
		System.out.println("Top Items Map:");
		for (Map.Entry<Integer, List<Crafting_Item>> entry : topItemsMap.entrySet()) {
			int score = entry.getKey();
			List<Crafting_Item> items = entry.getValue();
			System.out.println("Score: " + score);
			for (Crafting_Item item : items) {
				System.out.println(item); // Assuming Crafting_Item has a meaningful toString() implementation
			}
		}
