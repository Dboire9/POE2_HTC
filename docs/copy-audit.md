...
| 35 | "Messages aren't switched on right now" (was "Ratings …", shared since 2026-09-24) | `feedbackForm.tsx`, both panels | **OK** | 503: no `FEEDBACK_SENTRY_DSN`, one that is not a DSN, or BotID unable to run (the OIDC option off) — each is the site switched off, from the player's side |
| 35b | "What do you want next?" — "No email, no name, no account — only your words and the app version are sent" / "It helps decide what comes next." | `WhatNext.tsx`, guide | **OK** | The panel posts `{ topic: 'next', message, version, website: '' }` and nothing else (WhatNext.test.tsx pins the body); the function keeps the words and version only, and tags it `topic: next` / source `what-next` so it reads apart from ratings (feedback.test.ts). Same route, bot check and limits as rows 32–35 |
...
| 82 | "Not played out: too long a craft to play out — about N orbs and restarts a craft…" | `TabletResult.tsx`, `CostSpread.tsx`, from `markovReplay.ts` | **OK — measured** | The replay declines when its 64 minimum crafts would outrun 3× its clock (`GIVE_UP_AFTER`; 2 s on a tablet, 4 s on a gear play-out), or one craft passes 1,000,000 moves; N = moves over crafts played so far, or "over" the moves of the one still running when that is larger. Every move counts, a restart included. Ritual Effectiveness + Rarity + Unique Monsters' extra Rare modifier + extra Favour reroll: ~270,000, declined at ~6 s |
...
| 86 | "Craft N tablets and you come out ahead X% of the time — about P over the run. Have about B on hand…" / "Plan a run" table | `RunPlan.tsx` | **OK — resampled** | `planRuns` (profit.ts): 2,000 runs of up to 500 crafts, each craft's cost drawn from the replay's percentiles (interpolated), paid in full then sold at the typed price + average sold on the way. N = the shortest run ahead ≥ 90% of the time; P = N × (income − the replay's exact mean cost); B = the 90th percentile of the deepest a run is down before a sale. Seeded, so it does not change between renders |
| 87 | "How is this worked out?" — You spend / You get tables, "A − B = C profit a tablet" (or "an item"), "How sure: N crafts pin the average spend to within ±M, 19 times in 20" | `ProfitBreakdown.tsx` | **OK** | Spend, on a tablet: `spendBreakdown` — plain tablets = 1 + restarts + sales, each orb `movesPerCraft × sheet price`, the remainder (never counted as moves) = the Exalts filling the finished tablet; sums to plain + `replay.meanCost` exactly. On gear: `spendLines` — `replay.spendByMove` (each move as priced, an Echoes omen counted only when spent) + the white base (`restartCost`); sums to base + `replay.meanCost` exactly (solve.test.ts). "< 0.01" for a move played in under 1 craft of 200. Get: the typed price + `sales.perEntry[i] × typed price`. When nothing sells the verdict uses the solver's V and the panel says both. M = 1.96 × `replay.stdErr` |
...
| 89 | "Tip · buy a Magic one instead" / "Magic · one prefix, no suffix — worth up to X — more than a plain one / about a plain one / less than a plain one: only under this price" | `StandInTip.tsx` | **OK — from the solve** | `standIns` (tablets.ts): Magic with one junk prefix, Magic with one junk suffix (only those two are on the market in number), worth = plain + V(white) − V(state), V read off `markov.routes`; both always shown. Search: Magic + `FULL_USES` + `standInFilters` — one filter on the other side, "# Empty Suffix Modifiers" ≤ 1 for prefix-only and "# Empty Prefix Modifiers" ≤ 1 for suffix-only, as the site answers it (checked on the site, 2026-09-23) |
...
| 93 | "Craft along" / "Next: …" / "Use it in game, then pick what happened" / outcomes "X lands", "A suffix you did not ask for lands", "… — finished", "Nothing that helps — the item counts the same as before", each with its odds | `CraftAlong.tsx`, `stateWords.ts` | **OK — the policy** | `stepFor` → `stepFrom(routes, i)`: the state's move (`act`) and its realized outcomes (`outProb`), each worded by `changesBetween` on the two states, which is what the policy graph reads too. Shown only for an exact solve (`keepRoutes`, markovFromItem.ts) |
| 94 | "Spent so far X over N moves · still to spend Y on average — each move counted at the plan's average price for it" / "Picked up where you left off" / "✓ Finished. You spent about X; the plan put this craft at Y on average." | `CraftAlong.tsx`, `craftAlong.ts` | **OK** | spent = Σ `actCost` of the moves picked (the move's price plus, for an Echoes-omened Desecration, the omen's expected spend — hence "average"); "still to spend" = the state's V; the place is kept per tab in `poe2htc.craftAlong.v1` for the same craft (`craftSig`) |
| 95 | "It offers three modifiers and you keep one. Keep the first of these that is on offer" / "All three below this line? Throw them back with the Omen of Abyssal Echoes, then keep the best of the new three." / "Next: bin this item and start again on a new base — finishing this one would cost more than starting over." | `CraftAlong.tsx` | **OK — the solver's own rules** | Keep order = outcomes by V ascending: `keepWeights` keeps the cheapest-to-finish of an offer. The line = `rerollAbove` (`fresh + rerollCost` in `keepWeights`, hand-checked in markovEssenceDesecrate.test.ts); the omen throws an offer back once. Start over = the policy's `restart`, played only where `restartCost + V(start)` beats every other move |
| 96 | "What a craft can cost" / "Play it out" — "The average hides how far one craft can run past it. Play this plan out on real items — a few seconds more — and see the spread." | `CostSpread.tsx` | **OK — measured** | The button solves the request on screen again with `playOut` (solve.ts): the same MDP, then `replayPolicy` on a 4 s clock (`PLAYOUT_MILLIS`) up to 20,000 crafts. Measured on 8 gear crafts from white at Standard (validation.md): +4.0 s each, 226–10,916 crafts. Shown only for an exact result — the replay plays nothing else (markovFromItem.ts) |
| 97 | "Half the crafts cost less than X. The luckiest 1 in 10 costs under Y; the unluckiest 1 in 10 more than Z." / "N crafts played out, following the plan below — a long craft, so a rough read; each counts the B base it starts from." | `CostSpread.tsx` | **OK** | X, Y, Z = `costPercentiles[50]`, `[10]`, `[90]` (nearest rank over the crafts played) + `restartCost`. "A rough read" under 500 crafts, the Tablets tab's line. The 8 crafts' replay means sit within 2 standard errors of the solver's costs (validation.md) |
| 98 | "Your budget of B covers N% of crafts." | `CostSpread.tsx` | **OK** | `shareWithin(percentiles + base, B)`: the share of crafts played whose whole cost, base included, came in at or under the Budget field — nearest rank, whole percents |
| 99 | "Not played out: R. The average above stands on its own." | `CostSpread.tsx` | **OK** | R = `replayReason`, the replay's own refusal (row 82, or a move it cannot play). The button is not offered again: the same plan declines the same way |
| 100 | "Crafting it to sell? It sells for [price]" / "Profit per item: about P on average" · "Loss per item: about L on average — buying one is cheaper" | `CraftToSell.tsx`, `ProfitVerdict.tsx` | **OK** | P = typed price − (`restartCost` + V(start)): the white base the craft starts from and the solver's exact cost, as the Tablets tab counts its first tablet. Kept per item in this browser (`gearPrices`, keyed by base + each slot's modifiers at their tiers — what the trade search looks for) |
| 101 | "One craft pays for itself N% of the time — how often a craft costs less than the S it sells for." / "Plan a run" with items | `CraftToSell.tsx`, `RunPlan.tsx` | **OK** | N = `shareWithin(percentiles + base, S)`. The run plan is row 86's `planRuns` on the same percentiles, mean = base + `replay.meanCost` |
| 102 | "The carved base you start from is not counted: add what it cost you." / "Play it out, above, to see how often one craft pays for itself and how many to craft." / "The cost above is only a bound, so this cannot say yet whether crafting it pays — raise Search effort to settle it." | `CraftToSell.tsx` | **OK** | A carved craft has no `restartCost` (it never starts over), so the spend is V(start) alone. The prompt shows only where a play-out can run: an exact result the replay has not declined. No verdict on a bound (`bound !== 'exact'`): against a floor a loss can read as a profit, against a ceiling a profit as a loss |
| 103 | "⚠ Even starting from the best plan without Desecration, the solver ran out of time before this craft settled with it. This is that plan’s exact cost, and a ceiling: bones can only make the craft cheaper. The route below uses none — raise Search effort to find out how much cheaper." | `LabResults.tsx`, from `markovBoneFree.ts` | **OK — measured** | Shown when `withoutBones`: on a from-white craft whose bones are `optional`, the plan without bones was solved first and settled, it seeded the solve with bones, and that solve still ran out (`stoppedEarly`, or a ceiling above this one). "Only cheaper": a plan without Desecration is playable in the model with it, so V(with) ≤ V(without) (markovBoneFree.test.ts: 885.5 ≤ 928.5 on a frozen-sheet Wand). Rare now: seeded, every craft of the bone table settles at Standard (validation.md) |
| 104 | "Or try one" — "Pick modifiers above to plan your own craft — or load one of these and see what the app answers." | `ExampleCrafts.tsx` | **OK** | Each example (`exampleCrafts.ts`) is loaded by `craftFromScratch` and solved at once (`tryExample`). Chosen to settle exactly at Standard: measured 0.3–1.3 s on the live sheet (validation.md), held to settling within Standard's sweeps on the frozen sheet by `exampleCrafts.test.ts`, and in the browser by e2e 2c |
| 105 | Guide: "nothing you type leaves your machine, except a rating or a "What next?" message you choose to send" | `USER_GUIDE.md` | **OK — corrected 2026-09-24** | It said "nothing you type leaves your machine" and "no server" after `api/feedback.ts` existed. The only requests carrying typed words are `sendFeedback`'s (`feedbackForm.tsx`), on Send |
| 106 | Streamer gear: "their gear is a snapshot refreshed every week rather than live" (was "…we refresh by hand") | `StreamerGear.tsx` | **OK** | `.github/workflows/refresh-streamers.yml`, Mondays 06:30 UTC (`cron` pinned in deployConfig.test.ts): fetch, whole suite, merge when every streamer came back and the suite passed, else a held PR. A week with no gear change opens nothing, so the file's date is the last CHANGE, which the tab shows |
