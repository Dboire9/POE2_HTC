...
| 76 | "Profit per tablet: about X on average — counting C from good tablets you'd sell instead of binning" / "Loss per tablet … buying one is cheaper" | `TabletResult.tsx` | **OK — a LOWER bound on the credit, said as what it is** | Typed price − (plain tablet + V) + `binnedCredit`: tablets the plan restarts away while holding a watched set, each priced once at the best TYPED price among the sets it held (unpriced → 0). Selling mid-craft is not counted (TODO 25) |
