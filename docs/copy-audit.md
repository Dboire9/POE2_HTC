...
| 103 | "⚠ With Desecration in play the solver ran out of time on this craft, so it solved it once more without Desecration, past the time Standard allows. This is the exact cost of that plan, and a ceiling: bones can only make the craft cheaper. The route below uses none — raise Search effort to find out how much cheaper." | `LabResults.tsx`, from `markovBoneFree.ts` | **OK — measured** | Shown when `withoutBones`: the solve with bones ran out (`stoppedEarly`, or a ceiling above this one) on a from-white craft whose bones are `optional`, and the second solve — bones excluded, on the preset's own clock again, AFTER the first — settled. "Only cheaper": a plan without Desecration is playable in the model with it, so V(with) ≤ V(without) (markovBoneFree.test.ts pins it, 885.5 ≤ 928.5 on a frozen-sheet Wand). "Past the time": the second clock is extra (validation.md: Amulets 5×T2 21.5 s, Rings 5×T2 17.6 s at Standard's 15 s) |
