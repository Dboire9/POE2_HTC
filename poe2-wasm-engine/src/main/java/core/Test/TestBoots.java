...
public class TestBoots {
	...
	public static void main(String[] args) {
		...
		for (Modifier mod : possibleEssencesPrefixes) {
			System.out.println("Available essence prefix: " + mod.text);
			if (mod.text.equals("#% increased Movement Speed (Essence of Hysteria)")) {
				desiredMods.add(mod);
	...
	// }
		...
		try {
			...
			List<Probability_Analyzer.CandidateProbability> results = CraftingExecutor.runCrafting(item, desiredMods,
					undesiredMods, GLOBALTHRESHOLD / 100, new ArrayList<>());
	...
	/**
	 * Randomly selects up to 3 modifiers (prefixes or suffixes) from the given
	 * list.
		...
		for (Probability_Analyzer.CandidateProbability cp : results) {
			...
			for (Map.Entry<Crafting_Action, ModifierEvent> entry : cp.bestPath().entrySet()) {
				...
				if (event.modifier != null) {
					...
					String probStr = stepProbability != null ? String.format(" [%.2f%%]", stepProbability * 100) : "";
			...
			if (hasPhysicalFlat && hasPhysicalPercent && hasLightningAfterPhysical &&
				...
				System.out.println(
						"Final probability: " + probability + "% (" + String.format("%.2e", probability / 100) + ")");
				System.out.println("\nComplete sequence:");
	...
	private static void displayResults(List<Probability_Analyzer.CandidateProbability> results,
		...
		for (int i = 0; i < Math.min(1, results.size()); i++) {
			...
			for (Map.Entry<Crafting_Action, ModifierEvent> entry : cp.bestPath().entrySet()) {
				...
				System.out.println("Step " + (z + 1) + ": " + cp.candidate().modifierHistory.get(z));
				
				// Check if this is the third event and if it contains movement speed
				if (z == 2) {
					System.out.println("  [DEBUG] Third event modifier: " + event.modifier.text);
					System.out.println("  [DEBUG] Third event modifier source: " + event.modifier.source);
					System.out.println("  [DEBUG] Is it movement speed? " + event.modifier.text.contains("Movement Speed"));
				}
