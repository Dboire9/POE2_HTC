 ...
 * cache the URL. ~1200x630 of dark gradient compresses to well under Discord's limits either way.
 *
 * deviceScaleFactor 2 renders at 2400x1260 and lets the encoder downsample, which keeps the type edges
 * clean — rendering straight at 1200 leaves them visibly ragged once Discord scales to ~500px.
 */
...
const WIDTH = 1200;
const HEIGHT = 630; // declared in index.html's og:image:width / og:image:height — keep all three in step
...
console.log(`og.jpg ${WIDTH}x${HEIGHT} -> ${out} (${(statSync(out).size / 1024).toFixed(1)} kB)`);
