...
| 79 | "Selling what lands on the way: about R a craft … the craft costs about N net" / "you'd sell one in X% of crafts" | `TabletResult.tsx` | **OK — measured, with a stated rule** | `ReplayResult.sales`: the replay sells a priced watched set when its TYPED price beats `restartCost + V(start) − V(here)` (a one-step improvement on the solved policy, so never worse). N = plain tablet + replayed spend (incl. the fresh tablets sales force) − R |
