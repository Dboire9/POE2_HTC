...
- **The Omen of Whittling is a CHAOS omen, and it removes the lowest LEVEL modifier** — traced to poe2db 2026-09-02: *"your next Chaos Orb will remove the lowest level modifier"*, internal id `OmenOnChaosLowestLevelMod`. TODO 12 specified it as an Orb of Annulment omen removing the lowest TIER, and both halves were wrong; building it that way would have modelled a mechanic the game does not have. **Level, not tier**, matters because tier numbers are per-mod and not comparable across mods (a T5 of a five-tier mod is its worst roll, a T5 of a ten-tier mod is mid-range) — item level is the one scale the whole item shares, and every tier already carries its `ilvl`. `lowestLevelMods` / `chaosRemovalProbability` (`probability.ts`) model it; a FRACTURED mod is excluded from the removal pool, so it is never whittled AND its exclusion can move which mod counts as lowest. The TIE rule is untraced — uniform among the tied is modelled, matching every other pick-among-equals here, and flagged in docs/validation.md.
  ...
  **Whittling is the ONE default-excluded currency in the app** (`DEFAULT_EXCLUSIONS`, currencyPrefs.ts), and that is a product decision rather than a modelling one: the optimizer ranks BY cost and would decline the omen where it does not pay, so this is not correcting the maths — it chooses which question is asked by default ("what if I own none" over "what if I can buy one"). `WhittlingToggle` (SearchEffort.tsx) pins an on/off beside Search effort on both tabs; it is a VIEW of the same `omens.only` member the exclusions panel writes, never a second copy. Watch the group's semantics when editing it: an EMPTIED `only` means the whole omen group is excluded, so unticking the last member must delete the group, not empty it. The default applies only when storage holds nothing — a stored `{}` is a user who cleared their exclusions and must stay cleared.
