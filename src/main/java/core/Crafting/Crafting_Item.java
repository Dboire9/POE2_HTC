 ...
 * and manipulate crafting items, including copying, resetting, and adding
 * affixes.
 */
public class Crafting_Item {
	 ...
	 *
	 * @return A new `Crafting_Item` instance that is a deep copy of the current
	 *         instance.
	 */
	...
	/**
	 * Adds affixes to the crafting item based on the provided modifiers, actions,
	 * and undesired modifiers.
	 *
	 * @param mod           The list of modifiers to consider for adding.
	 * @param item          The crafting item to which affixes will be added.
	 * @param action        A map of crafting actions and their associated
	 *                      probabilities.
	 ...
	 */
	public List<Crafting_Item> addAffixes(List<Modifier> mod, Crafting_Item item, Map<Crafting_Action, Double> action,
		...
		for (Modifier m : mod) {
			...
			if (!Item_family.contains(m.family) && (undesiredMods == null || !undesiredMods.contains(m))) {
				...
				if (m.type == ModifierType.PREFIX && item.getAllCurrentPrefixModifiers().size() < 3) {
					...
					new_item.modifierHistory
							.add(new ModifierEvent(m, lowestTier, action, ModifierEvent.ActionType.ADDED));
	 ...
	 */
	public List<Crafting_Item> addPerfectEssenceAffixes(List<Modifier> mod, Crafting_Item item,
			Map<Crafting_Action, Double> action, List<Modifier> desiredMods) {
		List<Crafting_Item> Items_List = new ArrayList<>();
		List<String> Item_family = new ArrayList<>();
		for (Modifier currentAffixes : item.getAllCurrentModifiers())
			Item_family.add(currentAffixes.family);
		...
		for (Modifier m : mod) {
			...
			if (!Item_family.contains(m.family)) {
				...
				// If we want to apply a perfect essence that goes on a prefix but prefixes are
				// full, we only remove and add on prefixes
				if (m.type == ModifierType.PREFIX && item.getAllCurrentPrefixModifiers().size() >= 3)
					for (int i = 0; i < 3; i++) {
						...
						new_item_copy.modifierHistory.add(new ModifierEvent(m, lowestTier, action,
								ModifierEvent.ActionType.CHANGED, changed_modifier));
					...
					// We need to look for the modifier type and add the perfect essence only on it,
					// but removing all the affixes we can
					for (int i = 0; i < 3; i++) {
						Modifier changed_modifier;
	...
	public List<Crafting_Item> removeAffixes(Crafting_Item item, Map<Crafting_Action, Double> action) {
		...
		for (Modifier currentAffix : item.getAllCurrentModifiers()) {
			...
			// Skip removal if the current affix is the last modifier, there is no point in
			// removing a mod we just put on
			...
			if (currentAffix.source == ModifierSource.DESECRATED) // If we remove a desecrated mod, it can be appleid
																	// desecration again
				new_item.desecrated = false;

			// Record the removal event
			new_item.modifierHistory
					.add(new ModifierEvent(currentAffix, null, action, ModifierEvent.ActionType.REMOVED));
	...
	public void removePrefix(Modifier mod) {
		for (int i = 0; i < this.currentPrefixes.length; i++) {
			if (this.currentPrefixes[i] != null && this.currentPrefixes[i].text != null
					&& this.currentPrefixes[i].text.equals(mod.text)) {
				this.currentPrefixes[i] = null;
	...
	public void removeSuffix(Modifier mod) {
		for (int i = 0; i < this.currentSuffixes.length; i++) {
			if (this.currentSuffixes[i] != null && this.currentSuffixes[i].text != null
					&& this.currentSuffixes[i].text.equals(mod.text)) {
				this.currentSuffixes[i] = null;
